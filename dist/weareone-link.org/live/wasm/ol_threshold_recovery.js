 ...
 * @param {Uint8Array} xs
 * @param {Uint8Array} streams_flat
 * @param {number} secret_len
 * @param {number} k
 * @returns {Uint8Array}
 */
export function reconstructSecret(xs, streams_flat, secret_len, k) {
    const ptr0 = passArray8ToWasm0(xs, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passArray8ToWasm0(streams_flat, wasm.__wbindgen_malloc);
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.reconstructSecret(ptr0, len0, ptr1, len1, secret_len, k);
    if (ret[3]) {
...
export function splitSecret(secret, k, n) {
    const ptr0 = passArray8ToWasm0(secret, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.splitSecret(ptr0, len0, k, n);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('ol_threshold_recovery_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
